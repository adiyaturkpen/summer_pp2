...
NEXT_COIN_MILESTONE = 3      # Через сколько монет увеличивается скорость
...
# Класс врага (машины), движущегося сверху вниз
class Enemy(pygame.sprite.Sprite):
    ...
    # Функция внутри класса, отвечающая за движение врага вниз
    def move(self):
        global SCORE
        self.rect.move_ip(0, SPEED)
        # Если враг выходит за нижнюю границу, сбрасываем его вверх и прибавляем очко
        if self.rect.bottom > SCREEN_HEIGHT:
            SCORE += 1
            self.rect.top = 0
            self.rect.center = (random.randint(40, SCREEN_WIDTH - 40), 0)

# Класс игрока (машины игрока)
class Player(pygame.sprite.Sprite):
    ...
    # Функция внутри класса, отвечающая за движение игрока влево и вправо
    def move(self):
...
# Класс монеты, которую игрок может собирать
class Coin(pygame.sprite.Sprite):
    ...
    # Функция внутри класса, отвечающая за движение монеты вниз
    def move(self):
        self.rect.move_ip(0, SPEED)
        # Если монета уходит за экран, появляется снова сверху
...
# Создание групп для удобного управления спрайтами
...
while True:
    # Проверка событий (например, нажатие на крестик окна)
    ...
    # Отображение текущего счёта и количества собранных монет
    ...
    # Проверка столкновения игрока с врагом — если да, то конец игры
    ...
    if pygame.sprite.spritecollideany(P1, coins):
        COINS += 1
        for coin in coins:
            # Появление монеты снова сверху в случайной позиции
            coin.rect.top = 0
            coin.rect.center = (random.randint(40, SCREEN_WIDTH - 40), 0)

        # Каждые 3 собранные монеты увеличиваем скорость
        if COINS >= NEXT_COIN_MILESTONE:
            SPEED += 0.5
            NEXT_COIN_MILESTONE += 3
